...
export default function ManualTest() {
  ...
  const runTests = async () => {
    ...
    try {
      ...
      if (error) {
        testDetails.push(`❌ Auth connection failed: ${error.message}`)
      ...
      // Test 2: Check if we can query the auth.users table (built-in)
      setStatus('Testing auth system...')
      const { data: userData, error: userError } = await supabase.auth.getUser()
      
      if (userError) {
        testDetails.push(`⚠️  Auth user check: ${userError.message}`)
      } else {
        testDetails.push(`✅ Auth system working${userData.user ? ` (user: ${userData.user.email})` : ' (no user logged in)'}`)
      }
      
      // Test 3: Try to query our custom users table
